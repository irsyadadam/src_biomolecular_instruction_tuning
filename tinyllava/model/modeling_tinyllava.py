...
class TinyLlavaForConditionalGeneration(TinyLlavaPreTrainedModel):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if getattr(self.config, 'proteomics_mode', False):
            return self.prepare_inputs_labels_for_proteomics(
                input_ids, position_ids, attention_mask, past_key_values, labels,
                images, image_sizes  # 'images' contains proteomics data in this case
            )
        else:
    ...
    def prepare_inputs_labels_for_proteomics(
        self, input_ids, position_ids, attention_mask, past_key_values, labels,
        proteomics_data, proteomics_sizes=None
    ):
        """Prepare inputs for proteomics data (similar to image processing)"""
        vision_tower = self.vision_tower
        if vision_tower is None or proteomics_data is None or input_ids.shape[1] == 1:
            return input_ids, position_ids, attention_mask, past_key_values, None, labels

        # Encode proteomics data
        proteomics_features = self.encode_proteomics(proteomics_data)

        # Handle dummy tensors (similar to original image processing)
        _labels = labels
        _position_ids = position_ids
        _attention_mask = attention_mask
        ...
        new_input_embeds = []
        new_labels = []
        cur_proteomics_idx = 0
        
        for batch_idx, cur_input_ids in enumerate(input_ids):
            num_proteomics = (cur_input_ids == IMAGE_TOKEN_INDEX).sum()  # Reuse IMAGE_TOKEN_INDEX for proteomics
            
            if num_proteomics == 0:
                cur_proteomics_features = proteomics_features[cur_proteomics_idx]
                cur_input_embeds_1 = self.language_model.get_input_embeddings()(cur_input_ids)
                cur_input_embeds = torch.cat([cur_input_embeds_1, cur_proteomics_features[0:0]], dim=0)
                new_input_embeds.append(cur_input_embeds)
                new_labels.append(labels[batch_idx])
                cur_proteomics_idx += 1
                continue

            proteomics_token_indices = [-1] + torch.where(cur_input_ids == IMAGE_TOKEN_INDEX)[0].tolist() + [cur_input_ids.shape[0]]
            cur_input_ids_noim = []
            cur_labels = labels[batch_idx]
            cur_labels_noim = []
            
            for i in range(len(proteomics_token_indices) - 1):
                cur_input_ids_noim.append(cur_input_ids[proteomics_token_indices[i]+1:proteomics_token_indices[i+1]])
                cur_labels_noim.append(cur_labels[proteomics_token_indices[i]+1:proteomics_token_indices[i+1]])
            ...
            for i in range(num_proteomics + 1):
                cur_new_input_embeds.append(cur_input_embeds_no_im[i])
                cur_new_labels.append(cur_labels_noim[i])
                if i < num_proteomics:
                    cur_proteomics_features = proteomics_features[cur_proteomics_idx]
                    cur_proteomics_idx += 1
                    cur_new_input_embeds.append(cur_proteomics_features)
                    cur_new_labels.append(torch.full((cur_proteomics_features.shape[0],), IGNORE_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))
